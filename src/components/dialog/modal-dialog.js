...
export default class ModalDialog extends lightMixins(SlotCollectorMixin) {
    ...
    firstUpdated() {
        ...
        this.#dialog?.addEventListener('click', e => {
            const closeButton = e.target.closest('[data-role="close"]');
     ...
     * ```
     * return html`
     *     <button type="button" data-role="close" aria-label="Close">
     *        &times;
     *     </button>`;
     * ```
    ...
    renderHeader() {
        return html`<button type="button" data-role="close" aria-label="Close">&times;</button>`;
    }
    ...
    /**
     * Renders the modal dialog structure, including the header, content slot, and footer. The dialog is wrapped in a `<dialog>` element with appropriate ARIA attributes for accessibility.
     * @category rendering
